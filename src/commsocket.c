		...
		int nConNum)
{
	...
	{
		ret = Sck_ErrMalloc;
		vdebugwrite( __FILE__, __LINE__, "func sckCliet_init() err: malloc %d\n", ret);
		return ret;
	}
...
int sckCliet_getconn(void *handle, char *ip, int port, int *connfd)
{
	...
	{
		ret = errno;
		vdebugwrite( __FILE__, __LINE__, "func socket() err:  %d\n", ret);
		return ret;
	}
	...
	/*
	 ...
	 {
	 ret = errno;
	 vdebugwrite( __FILE__, __LINE__, "func connect() err:  %d\n", ret);
	 return ret;
	 }
	 */
	...
	if (ret < 0)
	{
		...
		{
			vdebugwrite( __FILE__, __LINE__, "func connect_timeout() err:  %d\n", ret);
		}
	}
...
int sckClient_send(void *handle, int connfd, unsigned char *data, int datalen)
{
	...
	if (ret == 0)
	{
		...
		{
			ret = Sck_ErrMalloc;
			vdebugwrite( __FILE__, __LINE__, "func sckClient_send() mlloc Err:%d\n ", ret);
			return ret;
		}
...
int sckClient_rev(void *handle, int connfd, unsigned char *out, int *outlen)
{
	...
	{
		ret = Sck_ErrParam;
		vdebugwrite( __FILE__, __LINE__, "func sckClient_rev() timeout , err:%d \n", Sck_ErrTimeOut);
		return ret;
	}
	...
	if (ret == -1)
	{
		vdebugwrite( __FILE__, __LINE__, "func readn() err:%d \n", ret);
		return ret;
	} else if (ret < 4)
	{
		ret = Sck_ErrPeerClosed;
		vdebugwrite( __FILE__, __LINE__, "func readn() err peer closed:%d \n", ret);
		return ret;
	}
...
int sckServer_init(const char *address, int port, int *listenfd)
{
	...
	{
		ret = errno;
		vdebugwrite( __FILE__, __LINE__, "func setsockopt() err:%d \n", ret);
		return ret;
	}
	...
	{
		ret = errno;
		vdebugwrite( __FILE__, __LINE__, "func bind() err:%d \n", ret);
		return ret;
	}
	...
	{
		ret = errno;
		vdebugwrite( __FILE__, __LINE__, "func listen() err:%d \n", ret);
		return ret;
	}
...
int sckServer_accept(int listenfd, int *connfd, void *addr, int timeout)
{
	...
	if (ret < 0)
	{
		...
		{
			ret = Sck_ErrTimeOut;
			vdebugwrite( __FILE__, __LINE__, "func accept_timeout() timeout err:%d \n", ret);
			return ret;
		} else
		{
			ret = errno;
			vdebugwrite( __FILE__, __LINE__, "func accept_timeout() err:%d \n", ret);
			return ret;
		}
	}
...
int sckServer_send(int connfd, unsigned char *data, int datalen, int timeout)
{
	...
	if (ret == 0)
	{
		...
		{
			ret = Sck_ErrMalloc;
			vdebugwrite( __FILE__, __LINE__, "func sckServer_send() mlloc Err:%d\n ", ret);
			return ret;
		}
		//将本地数据转换为网络数据  ;小端===》大端
		int netlen = htonl(datalen);
