...
int tcp_client(const char *address, unsigned short port)
{
	...
	if (port > 0)
	{
		vdebugwrite( __FILE__, __LINE__, "port=%d", port );
		...
		{
			ret = errno;
			vdebugwrite( __FILE__, __LINE__, "%s ;errorno=%d", strerror(ret), ret );
			ERR_EXIT("bind");
		}		
	}
...
int accept_timeout(int fd, struct sockaddr_in *addr, unsigned int wait_seconds)
{
	...
	{
		ret = errno;
		vdebugwrite( __FILE__, __LINE__, "func accept() err:%d \n", ret);
		return ret;
	}
...
int activate_nonblock(int fd)
{
	...
	{
		ret = flags;
		vdebugwrite( __FILE__, __LINE__, "func activate_nonblock() err:%d", ret);
		return ret;
	}
	/*
	 *  按位或，加上没有锁的状态
...
int deactivate_nonblock(int fd)
{
	...
	{
		ret = flags;
		vdebugwrite( __FILE__, __LINE__, "func deactivate_nonblock() err:%d", ret);
		return ret;
	}
		...
		unsigned int wait_seconds)
{
	...
	if (ret < 0 && errno == EINPROGRESS)
	{
		...
		{
			//vdebugwrite( __FILE__, __LINE__, "22222222222222222\n");
