...
static void  do_user_x(session_t *sess);
static void  do_pass_x(session_t *sess);
...
{
	int ret = 0;
	ftp_relply(sess, FTP_GREET, "(hcfilesvr 1.0)");
...
static void  do_user_x(session_t *sess)
{
	...
	if( strcmp(sess->arg, tunable_username) )
	{
		//用户不存在
		ftp_relply(sess, FTP_LOGINERR, "Login incorrect");
		vdebugwrite(__FILE__, __LINE__,"user not find, user=%s", sess->arg);
		return;
	}
	ftp_relply(sess, FTP_GIVEPWORD, "Please specify the password");
}
...
static void  do_pass_x(session_t *sess)
{
	if( strcmp(sess->arg, tunable_password) )
	{
		//密码错
		ftp_relply(sess, FTP_LOGINERR, "Login incorrect");
		//vdebugwrite(__FILE__, __LINE__,"wrong passwd, passwd=%s, tunable_password = %s",
		//sess->arg, tunable_password );
		return;
	}
...
static void  do_stat(session_t *sess)
{
	...
	ftp_frelply(sess, "hcfilesvr 1.0 - secure, fast, stable");
